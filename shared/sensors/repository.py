...
def get_data(redis: Session, sensor_id: int, _from: str, to: str, bucket: str, timescale: Timescale) -> schemas.Sensor:
    ...
    if bucket == "hour":
        ...
        SELECT avg(velocity), avg(temperature), avg(humidity), avg(battery_level), time_bucket('1h', last_seen, 'UTC', TIMESTAMPTZ '2020-01-01T00:00:00Z') 
        FROM sensor_data
        WHERE id = {sensor_id}
        GROUP BY time_bucket('1h', last_seen, 'UTC', TIMESTAMPTZ '2020-01-01T00:00:00Z') 
        WITH NO DATA;
        """
